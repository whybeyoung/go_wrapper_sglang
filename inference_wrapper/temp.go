...
func WrapperWrite(hdl unsafe.Pointer, req []comwrapper.WrapperData) (err error) {
	...
	fmt.Println("WrapperWrite inst:%v", toString(inst))
	fmt.Println("WrapperWrite req:%v", toString(req))
	if !inst.active {
	...
	case stopped := <-inst.stopQ:
		if stopped {
			...
			state := comwrapper.DataEnd
			responseContent, _ := responseContent(state, 0, "", "")
			responseUsage, _ := responseUsage(state, 0, 0)
			if responseContent != nil && responseUsage != nil {
				if err := inst.callback(inst.usrTag, []comwrapper.WrapperData{*responseContent, *responseUsage}, nil); err != nil {
					wLogger.Errorw("WrapperWrite callback error", "error", err, "sid", inst.sid)
					return err
				}
				wLogger.Infow("WrapperWrite sent end signal", "sid", inst.sid)
			}
			return nil
		}
	default:
	...
	for _, v := range req {
		...
		openaoMsgs := formatMessages(string(v.Data), promptSearchTemplate)
		thinking := false
		if isReasoningModel {
			lastMsg := openaoMsgs[len(openaoMsgs)-1]
			if lastMsg.Role != "assistant" {
				openaoMsgs = append(openaoMsgs, Message{
					Role:    "assistant",
		...
		streamReq := &openai.ChatCompletionRequest{
			Model:       "default",
			Messages:    convertToOpenAIMessages(openaoMsgs),
		...
		if isNeedPDInfo {
			// 添加PD相关参数
			streamReq.ExtraBody = map[string]interface{}{
		...
		senderrorContent := func(err error) {
		...
		go func(req *openai.ChatCompletionRequest, status comwrapper.DataStatus) {
			...
			if err != nil {
				...
				senderrorContent(err)
				return
			}
			fmt.Println("inst.callback %v", inst.callback)
			fmt.Println("firstFrameContent %v", firstFrameContent)
			...
			for {
				...
				default:
					...
					if response.Usage != nil {
						...
						// 创建usage数据结构
						status = comwrapper.DataEnd
						usageWrapperData, err := responseUsage(status, response.Usage.PromptTokens, response.Usage.CompletionTokens)
						if err != nil {
							wLogger.Errorw("WrapperWrite usage error", "error", err, "sid", inst.sid)
							// 发送错误响应
							errorContent := comwrapper.WrapperData{
								Key:      "content",
								Data:     []byte(fmt.Sprintf(`{"error": "%v"}`, err)),
						...
						if err != nil {
							wLogger.Errorw("WrapperWrite usage content error", "error", err, "sid", inst.sid)
							senderrorContent(err)
							close(done)
							return
						}
						responseData = append(responseData, *content, *usageWrapperData)
