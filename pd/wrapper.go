...
// 评分越小，负载越轻，越适合调度
// 只使用实时指标（队列深度 + KV cache），历史记忆由 EWMA 提供
func calculatePrefillLoadScore(item V1LoadItem) float64 {
	...
	return queueScore + kvScore + capacityPenalty
}
...
func WrapperWrite(hdl unsafe.Pointer, req []comwrapper.WrapperData) (err error) {
	...
	for _, v := range req {
		...
		// pdInfo 在 Decode 模式下需从 __kv_info 获取，若请求中无 __kv_info 则为 nil，解引用会 panic
		if pdInfo == nil {
			wLogger.Errorw("WrapperWrite pdInfo is nil, missing __kv_info in request (decode mode) or getPDInfo failed (prefill mode)", "sid", inst.sid)
			return fmt.Errorf("pdInfo is nil: decode 请求中缺少 __kv_info，或 prefill 模式下 getPDInfo 未返回有效值")
		}
...
func WrapperDestroy(hdl interface{}) (err error) {
	...
	// 先 abort 并关闭 Stream，让阻塞在 Recv() 的 goroutine 立即返回，否则会等到 context 1 小时超时才退出
